...
v=`psql -X -t -A -c "SELECT CASE WHEN POSITION ('HAWQ' in version()) > 0 AND POSITION ('Greenplum' IN version()) > 0 THEN 'hawq' WHEN POSITION ('HAWQ' in version()) = 0 AND POSITION ('Greenplum' IN version()) > 0 THEN 'gp' ELSE 'OTHER' END;"`

if [ "$v" == "hawq" ]; then
	search_path=public,pg_catalog,hawq_toolkit

	# get the release number of HAWQ
	r=`psql -X -t -A -c "SELECT REPLACE((SPLIT_PART(SUBSTR(version, POSITION ('HAWQ' IN version) + 5), ' ', 1)), '.', '') as release FROM version();"`
...
t=`date`
echo "Start: $t"
psql -X -t -A -c "SET search_path=$search_path; SELECT 'VACUUM ANALYZE  \"' || n.nspname || '\".\"' || c.relname || '\";' FROM pg_class c JOIN pg_namespace n ON c.relnamespace = n.oid WHERE n.nspname = 'pg_catalog' AND c.relkind = 'r'" | psql -e
t=`date`
echo "Finish: $t"
...
if [ "$v" == "hawq" ]; then
	if [ $r -ge 1300 ]; then
		...
		psql -X -t -A -c "SET search_path=$search_path; select 'ANALYZE  \"' || smischema || '\".\"' || smitable || '\";' from hawq_stats_missing" | psql -e
	fi
else
	psql -X -t -A -c "SET search_path=$search_path; SELECT 'ANALYZE  \"' || n.nspname || '\".\"' || c.relname || '\";'
	FROM pg_class c
...
t=`date`
echo "Start: $t"
vacuum_freeze_min_age=`psql -X -t -A -c "show vacuum_freeze_min_age;"`
psql -X -t -A -c "SET search_path=$search_path; SELECT 'VACUUM  \"' || n.nspname || '\".\"' || c.relname || '\";' FROM pg_class c join pg_namespace n ON c.relnamespace = n.oid WHERE age(relfrozenxid) > $vacuum_freeze_min_age AND c.relkind = 'r'" | psql -e
t=`date`
echo "Finish: $t"
...
if [ "$v" == "gp" ]; then
	...
	t=`date`
	echo "Start: $t"
	psql -X -t -A -c "SET search_path=$search_path; SELECT 'VACUUM  \"' || bdinspname || '\".\"' || bdirelname || '\";' FROM gp_bloat_diag WHERE bdinspname <> 'pg_catalog'" | psql -e
	t=`date`
	echo "Finish: $t"
	...
	t=`date`
	echo "Start: $t"
	psql -X -t -A -c "SET search_path=$search_path;SET client_min_messages TO WARNING; SELECT 'VACUUM \"' || schema_name || '\".\"' || table_name || '\";'
	FROM    (
	        SELECT n.nspname AS schema_name, c.relname AS table_name, (__gp_aovisimap_compaction_info(c.oid)).compaction_possible AS compaction_possible
	...
	GROUP BY schema_name, table_name;" | psql -e
	t=`date`
	echo "Finish: $t"
fi
